...
{
    [Header("生命值")]
    [Tooltip("Boss 最大生命值。归零后 Boss 战结束并通知 BossArena 解除封场。")]
    public int maxHealth = 12;

    [Tooltip("Boss 当前生命值，仅用于运行时调试观察。")]
    ...
    private void SpawnProjectile(Vector2 position, Vector2 direction)
    {
        ...
        bossProjectile.Initialize(direction, projectileSpeed, projectileLifetime, projectileDamage, projectileKnockback);
    }
    ...
    private void SpawnTrap(Vector2 position, Vector2 size)
    {
        ...
        bossTrap.Initialize(trapWarningDuration, trapActiveDuration, trapDamage, trapKnockback);
    }
